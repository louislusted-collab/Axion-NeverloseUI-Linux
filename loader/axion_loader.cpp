...
void InjectionFinished(GObject* source, GAsyncResult* result, gpointer user_data)
{
    ...
    gtk_widget_set_sensitive(state->button, true);
    gtk_button_set_label(GTK_BUTTON(state->button), "Inject");
    g_clear_error(&error);
    g_free(output);
    g_free(error_output);
...
void InjectClicked(GtkButton*, gpointer user_data)
{
    ...
    gtk_widget_set_sensitive(state->button, false);
    gtk_button_set_label(GTK_BUTTON(state->button), "Injecting…");
...
void Activate(GtkApplication* application, gpointer user_data)
{
    ...
    gtk_window_set_default_size(GTK_WINDOW(window), 420, 245);
    ...
    state->button = gtk_button_new_with_label("Inject");
    gtk_widget_add_css_class(state->button, "inject");
    gtk_widget_set_halign(state->button, GTK_ALIGN_FILL);
    g_signal_connect(state->button, "clicked", G_CALLBACK(InjectClicked), state);
    gtk_box_append(GTK_BOX(card), state->button);
